...
class Fab {
  ...
  print() {
    ...
    if (!this.isPrinting && _syncVizStream) {
      ...
      console.log('copying over command stream');
    ...
    if (this.commandStream.length > 0) {
      ...
      console.log(this.commandStream);
  ...
  parseGcode() {
    console.log('clearing vertices');
    this.vertices = [];
    _fab.commands.forEach((cmd) => { // maybe change back to commandsForRendering to save drawing lots of little lines for midimode
      let fullcommand = cmd;
      cmd = cmd.trim().split(" ");
