  ...
  return function () {
    if (!_once) {
      ...
      if (typeof fabDraw === "function") {
        ...
        _fab.firstMoveSubdivided = false;
        _fab.waitedForFirstLog = false;
        fabDraw();
        _fab.parseGcode();
...
class Fab {
  constructor(config = defaultPrinterSettings) {
    ...
    this.firstMoveSubdivided = false;
    this.waitedForFirstLog = false;
  ...
  subdivideMove(cmd, traceLineNum = null) {
    let segmentationLength = 0.6; // was 0.1 for slow moves, 0.5 for default 
  ...
  getRealtimePosition() {
    ...
    // const positionQuery = `M118 S{"RT_POS " ^ "X:" ^ move.axes[0].machinePosition + 0.5 ^ " Y:" ^ move.axes[1].machinePosition + 39 ^ " Z:" ^ move.axes[2].machinePosition - 102}`;
  ...
  print() {
    ...
    // For fabscription, try flooding the buffer with initial moves to account for initial errors in M118 response
    // if (this.fabscribe) {
    //   // move up and down by half a millimeter, 100 times
    //   let syncCommands = [];
    //   syncCommands.push('G90 ; sync'); // absolute position
    //   syncCommands.push('M83 ; sync'); // relative E
    //   syncCommands.push('G0 X200 Y100 Z100 F600'); // manually move to start, make this automatic
    //   syncCommands.push('G91 ; sync'); // relative positioning
    //   syncCommands.push('M83 ; sync'); // relative E
    //   for (let i = 0; i < 200; i++) {
    //     // move left and right a bunch of times
    //     syncCommands.push('G0 X0.01 F100 ; sync');
    //     syncCommands.push('G0 X-0.01 F100 ; sync');
    //     // if (i == 100) {
    //     //   syncCommands.push('; STARTYOURENGINES');
    //     // }
    //   }
    //   syncCommands.push('; STARTYOURENGINES');
    //   // Add the sync commands to the front of the printstream
    //   for (let i = syncCommands.length-1; i >= 0; i-- ) {
    //     this.commands.unshift(syncCommands[i]);
    //   }
    // }
    ...
    if (this.commandStream.length > 0) {
      if (this.midiMode) {
        // TODO: Get initial position to segment the first move
        // ADD BACK IN for jubilee, i took out to test in Marlin but unsure if its currently needed
        // var positionQuery = `M118 S{"RT_POS " ^ "X:" ^ move.axes[0].machinePosition ^ " Y:" ^ move.axes[1].machinePosition ^ " Z:" ^ move.axes[2].machinePosition}`;
        // this.commandStream.unshift(positionQuery);
      }
  ...
  // TODO: For testing a segmentation of initial move, not implemented yet
  delay = ms => new Promise(res => setTimeout(res, ms));

  runGCodeCommand(commandToSend) {
  ...
  printStream() {
    ...
    if (this.commandStream.length > 0) {
      this.isPrinting = true;
      // TODO: subdivide the first move?
      // var initialPos;
      // if (!this.firstMoveSubdivided) {
      //   // get the initial position
      //   const yourFunction = async () => {
      //     console.log('waiting...')
      //     await this.delay(5000);
      //     console.log('carry on')
      //     this.waitedForFirstLog = true;
      //   }
      //   yourFunction();
      //   // var initialPos = this.log[0].split(' ');
      // }

      let commandToSend = this.commandStream[0];


      // TODO: If sending sychronization commands, try only collecting data after that's complete
      // if (commandToSend == "; STARTYOURENGINES") {
      //   // starting collecting position data
      //   this.positionQueryIntervalID = setInterval(() => {
      //     this.getRealtimePosition();
      //   }, 20);
      ...
      //   this.commandStream.shift();
      //   commandToSend = this.commandStream[0];
      // }
  ...
  fabscription(commandToSend) {
    ...
    // add this back in to manually keep time and send stuff
    // if (Date.now() - this.lastRtPosSent > 1 && _fab.sentCommands.length > 5 && _fab.sentCommands[_fab.sentCommands.length - 1] != positionQuery && commandToSend != positionQuery) {
    //   _fab.commandStream.unshift(positionQuery);
    //   console.log('Im gonna unshift: ', positionQuery);
    // }

    // TODO: The following logic might not be necessary on Marlin with new data collection strategy
  ...
  onData() {
    ...
    if (_fab.serialResp.slice(-1) == "\n") {
      if (_fab.serialResp.search("ok") > -1) {
        ...
        // TODO: Might be able to remove this with new data collection strategy. Test with Marlin.
        ...
        // removing this for Jubilee tests
        // if (_fab.fabscribe) {
      ...
      if (_fab.serialResp.search(" Count ") > -1) {
        ...
        // TODO: Might be able to remove this with new data collection strategy. Test with Marlin.
        // Taking out for Jubilee testing
        // if (_fab.autoHomeComplete && _fab.bufferFilled) {
  ...
  moveToXY(x, y, v) {
    // Move directly to an absolue X & Y coordinate without extruding
    this.setAbsolutePositionXYZ();
    this._moveXYZE({ x: x, y: y, v: v });
  }
  ...
  circle(x, y, z, radius, speed, eMultiplier, segments = 50
  ) {
    this.moveTo(x + radius, y, z);
    for (var t = 0; t <= TWO_PI + TWO_PI/segments; t += TWO_PI/segments) {
      fab.moveExtrude(radius * cos(t) + x , radius * sin(t) + y, z, speed, eMultiplier, true);
    }
  }
