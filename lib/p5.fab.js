...
p5.prototype.createFab = function () {
  // Preserve serial connection on reloads?
  _fab = new Fab();
  return _fab;
};
  ...
  return function () {
    if (!_once) {
      ...
      if (typeof fabDraw === "function") {
        _fab.lastAsyncPosition = new Vector4(0, 0, 0, 0); // positions used to plan toolpaths
        _fab.asyncPosition = new Vector4(0, 0, 0, 0); // positions used to plan toolpaths
...
const defaultPrinterSettings = {
  baudRate: 115200,
  nozzleDiameter: 0.4, //change back to 0.4
...
class Fab {
  constructor(config = defaultPrinterSettings) {
    ...
    this.commandsForRendering = [];
    this.printer = config.name;
    this.lastAsyncPosition = new Vector4(0, 0, 0, 0); // positions used to plan toolpaths
  ...
  subdivideMove(cmd) {
    ...
    if (moveCommands.indexOf(code) > -1) {
      var moveCommand = new LinearMove(cmd);

      // dist between current and last positions
      let dist = sqrt((this.asyncPosition.x - this.lastAsyncPosition.x) ** 2 + (this.asyncPosition.y - this.lastAsyncPosition.y) ** 2 + (this.asyncPosition.z - this.lastAsyncPosition.z) ** 2)
      if (dist > 0.5) { // threshold in mm
        // get slope
        ...
        const numSections = floor(dist / 0.5); // divide into 'numSections' sections of threshold size
        let p_i = new p5.Vector(this.lastAsyncPosition.x, this.lastAsyncPosition.y, this.lastAsyncPosition.z);
        let p_f = new p5.Vector(this.asyncPosition.x, this.asyncPosition.y, this.asyncPosition.z);
        let i = 0;
        while (i < numSections) {
          let t = i / numSections;
          let p_ = p5.Vector.mult(m, t);
          // let subPt = p5.Vector.add(p_i, p_); // if i do this, strings are added? is this a bug?
          let subPt = new p5.Vector(parseFloat(p_i.x) + parseFloat(p_.x), parseFloat(p_i.y) + parseFloat(p_.y), parseFloat(p_i.z) + parseFloat(p_.z));
  ...
  parseGcode() {
    this.vertices = [];
    _fab.commandsForRendering.forEach((cmd) => { // maybe change back to commandsForRendering to save drawing lots of little lines for midimode
      let fullcommand = cmd;
      cmd = cmd.trim().split(" ");
  ...
  makeE(x, y, z) {
    ...
    return (
      dist3D(x, y, z) * (this.nozzleR / this.filamentR) ** 2
    ).toFixed(2);
    // return (
